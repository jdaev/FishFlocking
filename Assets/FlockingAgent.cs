...
public class FlockingAgent : MonoBehaviour
{
    ...
    Vector3 CalculateTargetVector()
    {
        ...
        else if (flock.Package.diet == Flock.Diet.Carnivorous || flock.Package.diet == Flock.Diet.Omnivorous)
        {
            foreach (Flock otherFlock in MainFlow.allFlocks)
            {
                if (otherFlock.Package.power < flock.Package.power)
                {
                    for (int i = 0; i < otherFlock.agents.Count; i++)
                    {
                        float distanceFromFood = Vector3.Distance(otherFlock.agents[i].transform.position, transform.position);

                        if (Vector3.SqrMagnitude(otherFlock.agents[i].transform.position - transform.position) < Mathf.Pow(flock.Package.foodPerceptionRange, 2))
                        {
                            if (Vector3.SqrMagnitude(mouth.position - otherFlock.agents[i].transform.position) <= Mathf.Pow(flock.Package.mouthSize, 2))
                            {
                                otherFlock.agents[i].KillAgent();
                            }
                            else
                                foodVector += (otherFlock.agents[i].transform.position - transform.position).normalized / Mathf.Clamp(distanceFromFood, 1f, flock.Package.foodPerceptionRange);
                        }
                    }
                }
            }
